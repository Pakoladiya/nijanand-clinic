...
export default function ReceptionPage({ navigateTo }: { navigateTo?: (page: string, patientId?: string) => void }) {
  ...
  const [subTab, setSubTab] = useState<'register' | 'waiting' | 'attended'>('waiting')
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      const sorted = [...(matched || [])].sort((a, b) =>
        (yestIds.has(a.id) ? 0 : 1) - (yestIds.has(b.id) ? 0 : 1)
      )
  ...
  async function addToQueue(patient: Patient) {
    ...
    await supabase.from('waiting_list').insert({
      patient_id: patient.id, date: today, session,
      added_by: staff.id, status: 'waiting', notes: '',
    })
              ...
              {queue.length === 0 ? (
                ...
                queue.map((entry, i) => (
                  <div key={entry.id}
                    ...
                    <button onClick={() => removeFromQueue(entry)}
